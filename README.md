...
Due to the large nature of my images, I received permission from Dr. Beveridge to use an external image-formatting library, as my output PPMs were approaching gigabytes in size.
...
## Extra Credit
For extra credit, I implemented a *bounding volume hierarchy*, *texture mapping*, and *smooth shading*.
...
This requires linking in `libpng`, but that's already installed on the school machines, so everything works out.
...
## Renderings
All of the custom renderings are located in `custom_drivers`.
...
The second custom driver, or the "wow" driver, is a modification of my previous model driver, and located in `custom_drivers/driver01.txt`
Make it by running:
...
It's also *really* large---`16000x9000` in resolution!
Still, even at this resolution it takes roughly 4 minutes to render on my machine.
